...
In der Tabelle places speichern wir die Orte und deren Koordinaten.
...
CREATE TABLE `places` (
    ...
    `LAT` DECIMAL(6,6) NOT NULL DEFAULT '0.000000',
    `LONG` DECIMAL(6,6) NOT NULL DEFAULT '0.000000',
...
Deren Emissionen lassen sich zwar berechnen, werden aber gerade im ländlichen Kontext mit der weiten Verbreitung von Holzöfen von deren Emissionen saisonal stark überlagert. Eine mess- oder sogar spürbare Veränderung durch die Reaktivierung der Steigerwaldbahn ist in dieser Schadstoffklasse bestenfalls an der Mainbrücke der B286 in Schweinfurt an warmen Sommertagen nachweisbar. Dort wäre jedoch nur ein kleiner Teil des Verkehrs und dessen Emissionen, eine Gesamtberechnung erübrigt sich daher.
...
\newline
- vermiedene Getötete: ~0.168 / Jahr (oder als Kehrbruch ausgelegt: wenn man die Steigerwaldbahn nicht reaktiviert, akzeptiert man, dass ca. alle 6 Jahre eine Person im Straßenverkehr im Steigerwaldvorland umkommt)\newline
- vermiedene Unfälle mit Personenschäden: ~16 / Jahr\newline
- vermiedene Verletzte: ~ 21 / Jahr.\newline
\newline
...
\begin{tabular}{ l  l  l }
\hline
vermiedene Betriebskosten an Werktagen [T€] & vermiedene Betriebskosten  an Nicht-Werktagen [T€] & vermiedene Betriebskosten  Ganzjährig [T€]\\ 
\hline
-17388 & -4050 & -21438\\ 
\end{tabular}
\newline
...
\begin{minted}{sql}
SELECT 
ROUND(SUM(t1.gesamtfahrleistung) * 249 * 0.00055) AS `vermiedene Betriebskosten an Werktagen [T€]`, 
ROUND(SUM(t1.gesamtfahrleistung) * 0.5 * 116 * 0.00055) AS `vermiedene Betriebskosten  an Nicht-Werktagen [T€]`, 
ROUND(
